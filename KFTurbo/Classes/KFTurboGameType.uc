...
function InitGame(string Options, out string Error)
{
    ...
    {
        class'KFTurboGameTypeHelper'.static.FixUpClientMovers(self);
    }
...
function ChangeName(Controller Other, string NewName, bool bNameChange)
{
    local Controller C;
	...
	NewName = StripColor(NewName);

    if (Other.PlayerReplicationInfo.playername ~= NewName)
    {
        return;
    }

    if (class'KFTurboMut'.default.bFixPlayerNameEncoding)
    {
        NewName = class'KFTurboGameTypeHelper'.static.FixUTFEncoding(NewName);
    }
